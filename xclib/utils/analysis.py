...
def _as_string(ind, vals):
    ...
    if vals is None:
        ...
        return ", ".join(["{}: {:.2f}".format(item[0], item[1]) for item in zip(ind, vals)])
...
def compare_predictions(docs, labels, true_labels, predicted_labels, 
    ...
    for _, idx in enumerate(sample_indices):
        ...
        for key, val in predicted_labels.items():
            ...
            _pred += "{}: {}\n\n".format(key, _as_string(itemgetter(*_ind)(labels), _score))

        print("text: {}\ntrue labels: {}\n\n{}-----\n".format(docs[idx], _true, _pred))


def compare_nearest_neighbors(tr_embedding, tr_text, ts_embedding=None,
                      ts_text=None, num_neighbours=10, sample_indices=None,
                      num_samples=10, method='brute', space='cosine', 
                      num_threads=-1):
